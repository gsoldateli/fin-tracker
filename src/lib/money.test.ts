...
import { parseRealToCents, formatCentsToReal, pushDigit, popDigit } from "./money";

describe("parseRealToCents", () => {
  it('parses "19,90" to 1990', () => {
    expect(parseRealToCents("19,90")).toBe(1990);
  });

  it('parses "1.234,56" to 123456', () => {
    expect(parseRealToCents("1.234,56")).toBe(123456);
  });

  it('parses "-50" to -5000', () => {
    expect(parseRealToCents("-50")).toBe(-5000);
  });

  it('parses "abc" to null', () => {
    expect(parseRealToCents("abc")).toBeNull();
  });

  it('parses empty string to null', () => {
    expect(parseRealToCents("")).toBeNull();
  });
  ...
  it('parses "19,9" (one decimal digit) to 1990', () => {
    expect(parseRealToCents("19,9")).toBe(1990);
  });

  it('rejects more than 2 decimal places', () => {
    expect(parseRealToCents("19,999")).toBeNull();
  });

  it("rejects multiple commas", () => {
    expect(parseRealToCents("19,90,00")).toBeNull();
  });
  ...
  it('parses "0,01" to 1', () => {
    expect(parseRealToCents("0,01")).toBe(1);
  });

  it('parses "0,00" to 0', () => {
    expect(parseRealToCents("0,00")).toBe(0);
  });

  it('parses "1.234" (no decimal) to 123400', () => {
    expect(parseRealToCents("1.234")).toBe(123400);
  });

  it('parses "1234" (no separator) to 123400', () => {
    expect(parseRealToCents("1234")).toBe(123400);
  });

  it('parses "-0,50" to -50', () => {
    expect(parseRealToCents("-0,50")).toBe(-50);
  });

  it("rejects non-numeric integer part", () => {
    expect(parseRealToCents("abc,50")).toBeNull();
  });

  it("rejects non-numeric decimal part", () => {
    expect(parseRealToCents("10,ab")).toBeNull();
  });
...
describe("formatCentsToReal", () => {
  it('formats 1990 to "19,90"', () => {
    expect(formatCentsToReal(1990)).toBe("19,90");
  });

  it('formats -5000 to "-50,00"', () => {
    expect(formatCentsToReal(-5000)).toBe("-50,00");
  });

  it('formats 0 to "0,00"', () => {
    expect(formatCentsToReal(0)).toBe("0,00");
  });

  it('formats 123456 to "1.234,56"', () => {
    expect(formatCentsToReal(123456)).toBe("1.234,56");
  });

  it('formats -1 to "-0,01"', () => {
    expect(formatCentsToReal(-1)).toBe("-0,01");
  });

  it('formats 100 to "1,00"', () => {
    expect(formatCentsToReal(100)).toBe("1,00");
  });
  ...
  it('formats 100000 to "1.000,00"', () => {
    expect(formatCentsToReal(100000)).toBe("1.000,00");
  });

  it('formats 5 to "0,05" (single-digit cent)', () => {
    expect(formatCentsToReal(5)).toBe("0,05");
  });

  it('formats 1234567 to "12.345,67"', () => {
    expect(formatCentsToReal(1234567)).toBe("12.345,67");
  });
});
